...
按记录中的face_id取共同131个已检测GT，得到以下跨轮辅助比较：
...
已根据补充的原始 `crop_vfoa_faces.py` 确认，旧/新原图哈希采用不同的输入定义：

| 字段 | SHA-256的输入 |
| --- | --- |
| 旧manifest：`upright_rgb_sha256` | 宽高文本（如 `640,480:`）的UTF-8字节 + EXIF方向校正后的RGB像素字节 |
| 本轮：`rgb_sha256` | EXIF方向校正后的RGB像素字节 |

**这两列不能直接比较。此前将66张图的不同哈希列为原图一致性疑点，属于核验口径错误，现予以更正。** 即使像素相同，两种输入定义也会产生不同摘要；该差异不能作为图片被修改的证据，不需要因此重新检测、训练或推理，现有评价数值不变。原始裁剪脚本已归档在 `historical/crop_vfoa_faces.py`。

本次确认的是代码中的哈希定义，没有重新读取两轮全部原图核验像素；旧预测CSV也不独立证明当时checkpoint与本轮权重字节相同。因此上表描述按ID连接的预测变化，仍不能严格归因“仅裁剪改变导致性能变化”。

## 10. 已核验内容与尚未核验内容

已完成：

- 已从原始裁剪脚本和共享框脚本确认两种原图哈希定义；撤销对不可比摘要的直接一致性判断。
...
原始GT裁剪来自 `crop_vfoa_faces.py --dataset ... --output ... --splits train val`。原脚本现已保留为 `historical/crop_vfoa_faces.py`；其哈希包含宽高文本前缀，具体定义见第9节。manifest、训练脚本、GazeTR适配器和配置也已保留。
...
| analysis/source_hash_comparison.csv | 不同定义的原图哈希并列记录，不用于直接一致性判断 |
...
1. 哈希口径已核实并更正记录，保留已完成的三个分支实验，无需因此重跑。
